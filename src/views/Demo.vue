<template>
  <div class="hello">
    ...
    <ul>
      <li><div class="color hot"></div></li>
      <li><div class="color cloudy"></div></li>
      <li><div class="color breezy"></div></li>
      <li><div class="color night"></div></li>
      <li><div class="color stormy"></div></li>
      <br>
    </ul>
    <ul>
      <li v-for="(s, index) in statistics" v-bind:key="index">{{s.Group}}-{{s.Key}}-{{s.Value}}</li>
    </ul>
  </div>
</template>
  ...
  async mounted () {
    try {
      ...
      var result = await axios({
        method: 'POST',
        url: 'https://127.0.0.1:4000/api/query',
      ...
      this.statistics = result.data.data.statistics
    } catch (error) {
      console.error(error)
    }
  }
...
.cloudy {
  background: linear-gradient(to top right, rgba(63,81,181,1) 0%, rgba(3,155,229,1) 80%);
